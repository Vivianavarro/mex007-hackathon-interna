...
//cambie esta seccion//
    dataAllMovies.forEach((dataAllMovies) => {
        let poster = `<div class ="wrapper btnn" data-toggle="modal" data-target="#miModal${dataAllMovies.Poster}"> <!-- cambie esta linea-->
        ...
        <div class="modal fade" id="miModal${dataAllMovies.Poster}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <p class=gender>${dataAllMovies.Title}</p>
        <p class=gender>${dataAllMovies.Director}</p>
        <p class=gender>${dataAllMovies.Ratings[0].Value}</p>
        </div>`;
